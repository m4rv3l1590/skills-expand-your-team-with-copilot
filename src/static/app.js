  ...
  function displayFilteredActivities() {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty !== "") {
        // Specific level selected — only show activities that have a matching difficulty.
        // Activities with no difficulty set (open to all) are excluded from level-specific views.
        if (!details.difficulty || details.difficulty !== currentDifficulty) {
          return;
        }
      }
      // When currentDifficulty is "" (All), show every activity regardless of difficulty
